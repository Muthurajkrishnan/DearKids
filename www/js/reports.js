...
//For Get Admin Settings
    function get_tank_report(downL) {
		...
		if(downL != ""){
			$('#downLoad').val(1);
			$('#reportForm').attr('action',config_url + "export2.php");
			$('#reportForm').submit();
			return false;
		}
        $.ajax({
			...
			success: function (data) {
				...
				if(Object.size(Tdata) > 0){
					...
					for(var k=1; k<3; k++){
						...
						$('#bodyReport').append('<tr><td scope="row">'+NameArr[k]+'</td><td scope="row">'+Tdata[k][0].toFixed()+'</td><td class="text-right">'+Tdata[k][1].toFixed()+'</td><td class="text-right">'+Tdata[k][2].toFixed()+'</td><td class="text-right">'+Tdata[k][3].toFixed()+'</td></tr>');
